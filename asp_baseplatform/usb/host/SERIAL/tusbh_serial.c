...
tusbhCDCProcess(TUSBH_Device_t *pdevice, uint8_t *mes)
{
	...
	case CDC_PROCESS_IDLE:
		...
		else if(hcdc->rw_status != TUSBH_E_BUSY)
			hcdc->state = CDC_PROCESS_ERROR;
		else if(URB_Status == TUSBH_URB_DONE){
			if(mes[2] == hcdc->InPipe){
				hcdc->RxTrnPktSize = tusbhHDTrasLength(phost, hcdc->InPipe);
				cmd = CDC_LINE_STATUS_RECEIVED;
				hcdc->RxTrnLength += hcdc->RxTrnPktSize;
				hcdc->pRxTrnBuff += hcdc->RxTrnPktSize;
				hcdc->state = CDC_PROCESS_IDLE;
			}
			else{
				cmd = CDC_LINE_STATUS_SENDED;
				hcdc->TxTrnLength += hcdc->TxTrnPktSize;
				hcdc->pTxTrnBuff += hcdc->TxTrnPktSize;
				if(hcdc->TxTrnSize > hcdc->TxTrnLength)
	...
	case CDC_PROCESS_SEND:
		length = hcdc->TxTrnSize - hcdc->TxTrnLength;
		if(length > hcdc->OutEpSize)
			hcdc->TxTrnPktSize = hcdc->OutEpSize;
		else
			hcdc->TxTrnPktSize = length;
		tusbhBulkWrite(pdevice, hcdc->pTxTrnBuff, hcdc->TxTrnPktSize, hcdc->OutPipe);
		hcdc->state = CDC_PROCESS_IDLE;
		break;
	case CDC_PROCESS_RECEIVE:
		length = hcdc->RxTrnSize - hcdc->RxTrnLength;
		if(length > hcdc->InEpSize)
			hcdc->RxTrnPktSize = hcdc->InEpSize;
		else
			hcdc->RxTrnPktSize = length;
		tusbhBulkRead(pdevice, hcdc->pRxTrnBuff, hcdc->RxTrnPktSize, hcdc->InPipe);
		hcdc->state = CDC_PROCESS_IDLE;
		break;
...
tusbhCdcSetLineCoding(TUSBH_Handle_t *phost, uint8_t unit, Line_Coding_t *linecod)
{
	...
	if(pdevice->dstate == DEVICE_CLASS){
		...
		tusbSendData(phost->process_event, TUSBH_CLASS_EVENT, pdevice->idx, CDC_PROCESS_SET_LINE_CODING, 0);
	}
	return TUSBH_E_OK;
}
...
tusbhCdcReceive(TUSBH_Handle_t *phost, uint8_t unit, uint8_t *pbuf, uint32_t *length, uint32_t timeout)
{
	 ...
	 *  受信データ設定
	 */
	hcdc->RxTrnSize   = *length;
	hcdc->RxTrnLength = 0;
	hcdc->pRxTrnBuff  = pbuf;
	hcdc->rw_status = TUSBH_E_BUSY;
	hcdc->rw_taskid = tusbGetTaskID();
	tusbSendData(phost->process_event, TUSBH_CLASS_EVENT, pdevice->idx, CDC_PROCESS_RECEIVE, 0);
...
tusbhCdcSend(TUSBH_Handle_t *phost, uint8_t unit, uint8_t *pbuf, uint32_t length)
{
	 ...
	 *  送信データ設定
	 */
	hcdc->TxTrnSize   = length;
	hcdc->TxTrnLength = 0;
	hcdc->pTxTrnBuff  = pbuf;
	hcdc->rw_status = TUSBH_E_BUSY;
	hcdc->rw_taskid = tusbGetTaskID();
	tusbSendData(phost->process_event, TUSBH_CLASS_EVENT, pdevice->idx, CDC_PROCESS_SEND, 0);
