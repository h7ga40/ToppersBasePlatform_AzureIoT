...
#define PRT_REQUEST_PORTSTATUS      0
#define PRT_REQUEST_DEVICEID        1
#define PRT_REQUEST_SOFTRESET       2

#define PRT_MAX_PORTSTATUS_SIZE     256
...
typedef struct
{
	...
	uint8_t                   cmd;
	uint8_t                   dummy[2];
	uint16_t                  TxTrnPktSize;
	uint16_t                  RxTrnPktSize;
	...
	uint32_t                  TxTrnLength;
	uint32_t                  TxTrnSize;
	uint8_t                   *pTxTrnBuff;
	uint32_t                  RxTrnLength;
	uint32_t                  RxTrnSize;
	uint8_t                   *pRxTrnBuff;
	uint32_t                  timer;
	uint32_t                  urb_wait;
	uint32_t                  rwt_status;
	uint32_t                  rwr_status;
	uint8_t                   buffer[PRT_MAX_PORTSTATUS_SIZE];
} PRT_Handle_t;
...
TUSBH_ERCODE tusbhPrtPortStatus(TUSBH_Handle_t *phost, uint8_t unit, uint8_t *buf, uint32_t length);
TUSBH_ERCODE tusbhPrtSend(TUSBH_Handle_t *phost, uint8_t unit, uint8_t *pbuf, uint32_t length);
TUSBH_ERCODE tusbhPrtReceive(TUSBH_Handle_t *phost, uint8_t unit, uint8_t *pbuf, uint32_t length);
void tusbhSetPrtCallBack(TUSBH_Handle_t *phost, void (*func)(TUSBH_Device_t *p, uint8_t, TUSBH_ERCODE, uint32_t));
