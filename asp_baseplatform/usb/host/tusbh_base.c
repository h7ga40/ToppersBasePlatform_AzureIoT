	...
	for(;;){
		...
		if(rercd == E_OK){
			...
			case DEVICE_DISCONNECTED:
				 ...
				 *  USB HOSTクラスの終了処理
				 */
				syslog_1(LOG_NOTICE, "USBH device disconnect(%d) !", pdevice->idx);
				if(pdevice->pClass != NULL){
...
tusbhHandleConnect(TUSBH_Handle_t *phost, uint8_t hub, uint8_t port, uint8_t prm)
{
	...
	if(phost->numClass == 0)
		...
		if(pdevice->pre_connected == 0){
			syslog_4(LOG_INFO, "connect pdevice[%08x] hub(%d) port(%d) speed(%d) !", pdevice, hub, port, prm);
			pdevice->hub  = hub;
			pdevice->port = port;
			pdevice->activate = 1;
			...
			/*
			 *  接続後 CONNECT_WAIT_TIME ms待ち
			 */
			tusbDelay(CONNECT_WAIT_TIME);
			if(hub == 0){
...
tusbhEventTask(intptr_t exinf)
{
	...
	for(;;){
		...
		if(ercd == E_OK){
			syslog_4(LOG_INFO, "usbh_task evt(%d) [%02x][%02x][%02x] !", mes[0], mes[1], mes[2], mes[3]);
