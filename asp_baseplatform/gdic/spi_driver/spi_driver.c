...
spi_start(SPI_Setup_t *psps)
{
	...
	SdHandle.cs2_port = psps->cs2_pin;

	memset(&spi_initd, 0, sizeof(SPI_Init_t));
...
sdcard_lock(SDCARD_Handler_t *hsd)
{
	...
	digitalWrite(hsd->cs2_port, PORT_LOW);
	return ercd;
}
...
{
	ER ercd = E_OK;
	digitalWrite(hsd->cs2_port, PORT_HIGH);
	if(hsd->spi_lock != 0)
 ...
 */
void
sdcard_setspi2(ID portid, SPI_Handle_t *hspi, uint8_t cs_port)
{
	SdHandle.hspi     = hspi;
	SdHandle.cs2_port = cs_port;
}
...
#if 1	/* ROI DEBUG */
	syslog_4(LOG_NOTICE, "## resp 0[%02x] 1[%02x] 2[%02x] 3[%02x] ##", resp[0], resp[1], resp[2], resp[3]);
#endif	/* ROI DEBUG */
	sdcard_unlock(hsd);
