...
static const uint8_t
  Gcmd[] = {                  // Initialization commands for 7789 screens
 ...
 */
ER
lcd_writedata(LCD_Handler_t *hlcd, uint8_t *p, uint32_t len)
{
	ER ercd = E_OK;
...
#if SPI_WAIT_TIME != 0
	ercd = spi_transmit(hlcd->hspi, (uint8_t*)p, len);
#else
	if((ercd = spi_transmit(hlcd->hspi, (uint8_t*)p, len)) == E_OK){
		ercd = spi_wait(hlcd->hspi, 100);
	}
#endif
 ...
 */
static void
displayInit(LCD_Handler_t *hlcd, const uint8_t *addr)
{
	...
	while(numCommands--) {                 // For each command...
		...
		while(numArgs--) {                   //   For each argument...
			data[0] = *addr++;
			SVC_PERROR(lcd_writedata(hlcd, data, 1));  //     Read, issue argument
		}
 ...
 */
static void
commonInit(LCD_Handler_t *hlcd, uint16_t width, uint16_t height)
{
	hlcd->colstart  = hlcd->rowstart = 0; // May be overridden in init func
	hlcd->lcd_width = width;
	hlcd->lcd_height = height;
	if(width == 240 && height == 240){	/* 1.3" and 1.54 displays */
		hlcd->lcd_colstart = 0;
		hlcd->lcd_rowstart = 80;
	}
	else if(width == 135 && height == 240){	/*1.13" display */
		hlcd->lcd_colstart = 53;
		hlcd->lcd_rowstart = 40;
	}
	else{
		hlcd->lcd_colstart = 0;
		hlcd->lcd_rowstart = 0;
...
lcd_setAddrWindow(LCD_Handler_t *hlcd, uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1)
{
	...
	SVC_PERROR(lcd_writecommand(hlcd, ST7789_CASET)); // Column addr set
	data[0] = (uint8_t)((x0 + hlcd->colstart) >> 8);
	data[1] = (uint8_t)(x0 + hlcd->colstart);		// XSTART
	data[2] = (uint8_t)((x1 + hlcd->colstart) >> 8);
	data[3] = (uint8_t)(x1 + hlcd->colstart);		// XEND
	SVC_PERROR(lcd_writedata(hlcd, data, 4));

	SVC_PERROR(lcd_writecommand(hlcd, ST7789_RASET)); // Row addr set
	data[0] = (uint8_t)((y0 + hlcd->rowstart) >> 8);
	data[1] = (uint8_t)(y0 + hlcd->rowstart);		// YSTART
	data[2] = (uint8_t)((y1 + hlcd->rowstart) >> 8);
	data[3] = (uint8_t)(y1 + hlcd->rowstart);		// YEND
	SVC_PERROR(lcd_writedata(hlcd, data, 4));

	SVC_PERROR(lcd_writecommand(hlcd, ST7789_RAMWR)); // write to RAM
}

/*
 *  オリエンテーション設定
 *  param1  hlcd: Pointer to LCD Handler
 *  param2  m: orientation value(0-3)
 */
void
lcd_setRotation(LCD_Handler_t *hlcd, uint8_t m)
{
	...
	hlcd->rotation = m & 3; // can't be higher than 3
	madctl[0] = 0;

	switch(hlcd->rotation){
	case 0:
		madctl[0]  = ST7789_MADCTL_MX | ST7789_MADCTL_MY | ST7789_MADCTL_RGB;
		hlcd->colstart = hlcd->lcd_colstart;
		hlcd->rowstart = hlcd->lcd_rowstart;
		hlcd->_width  = hlcd->lcd_width;
		hlcd->_height = hlcd->lcd_height;
		break;
	case 1:
		madctl[0]  = ST7789_MADCTL_MY | ST7789_MADCTL_MV | ST7789_MADCTL_RGB;
		hlcd->colstart = hlcd->lcd_rowstart;
		hlcd->rowstart = hlcd->lcd_colstart;
		hlcd->_height = hlcd->lcd_width;
		hlcd->_width  = hlcd->lcd_height;
		break;
	case 2:
		madctl[0]  = ST7789_MADCTL_RGB;
		if(hlcd->lcd_width == 135 && hlcd->lcd_height == 240){
			hlcd->colstart = hlcd->lcd_colstart-1;
			hlcd->rowstart = hlcd->lcd_rowstart;
		}
		else{
			hlcd->colstart = 0;
	...
	SVC_PERROR(lcd_writecommand(hlcd, ST7789_MADCTL));
	SVC_PERROR(lcd_writedata(hlcd, madctl, 1));
}

/*
 *  ST7789 screensの初期化関数
 */
void
lcd_init(LCD_Handler_t *hlcd, uint16_t width, uint16_t height)
{
	commonInit(hlcd, width, height);
...
{
  SVC_PERROR(lcd_writecommand(hlcd, i ? ST7789_INVOFF : ST7789_INVON));
}
