 ...
 *  の責任を負わない．
 * 
 *  $Id: lcdshield.c 2055 2020-03-02 03:39:28Z coas-nagasima $
 */
...
main_task(intptr_t exinf)
{
	...
	lcd_DisplayStringAt(&DrawProp, 0, 20, (uint8_t *)"TBP TEST", CENTER_MODE);
	DrawProp.TextColor = ST7735_BLACK;
	DrawProp.BackColor = ST7735_WHITE;
	DrawProp.pFont = &Font8;
	lcd_DisplayStringAt(&DrawProp, 0, 80, (uint8_t *)"Helle World !", CENTER_MODE);
	...
	while(sens > 0){
		...
		if(i == 0 || i == NUM_JSPOSITION)
			...
			lcd_DisplayStringAt(&DrawProp, 0, 100, (uint8_t *)pos_str[i-1], CENTER_MODE);
#else	/* LOCAL DEFINE */
