 ...
 *  の責任を負わない．
 * 
 *  $Id: i2ctest.c 2055 2020-03-02 03:39:28Z coas-nagasima $
 */
...
void main_task(intptr_t exinf)
{
...
#ifdef EEPROM_ADDR
	syslog_1(LOG_NOTICE, "READ EEPROM [%08x] !", aRxBuffer2);
	if(i2c_memread(hi2c, (uint16_t)EEPROM_ADDR, 0, I2C_MEMADD_SIZE_16BIT, (uint8_t*)aRxBuffer2, RXBUFFERSIZE, 500)!= E_OK){
	    /* I2C RECEIVE ERROR */
	...
	do{
		/* STATUS READ */
		if(i2c_memread(hi2c, (uint16_t)ADT7410_ADDR, ADT7410_STATUS, I2C_MEMADD_SIZE_8BIT, (uint8_t*)aRxBuffer, 1, 500)!= E_OK){
			/* I2C RECEIVE ERROR */
	...
	while(count < 1000){
		if(i2c_memread(hi2c, (uint16_t)ADT7410_ADDR, ADT7410_TEMP_H, I2C_MEMADD_SIZE_8BIT, (uint8_t*)aRxBuffer, 1, 500)!= E_OK){
			/* I2C RECEIVE ERROR */
			syslog_1(LOG_ERROR, "## I2C ERROR(14)[%08x] ##", hi2c->ErrorCode);
			goto stop_task;
		}
		temp = aRxBuffer[0] << 8;
		if(i2c_memread(hi2c, (uint16_t)ADT7410_ADDR, ADT7410_TEMP_L, I2C_MEMADD_SIZE_8BIT, (uint8_t*)aRxBuffer, 1, 500)!= E_OK){
			/* I2C RECEIVE ERROR */
