...
#ifndef USB_OTG_FS_WAKEUP_EXTI_LINE
...
usbo_init(ID portid, USB_OTG_Init_t *pini)
{
	...
	if(portid == USB1_PORTID){	/* USB OTG FSポート */
		 ...
		 *  GPIO/USBポートクロック起動
		 */
		sil_orw_mem((uint32_t *)ENABLE_FS_BASE1, ENABLE_FS_VAL1);
#if defined(ENABLE_FS_BASE2)
		sil_orw_mem((uint32_t *)ENABLE_FS_BASE2, ENABLE_FS_VAL2);
#endif
		...
		gpio_setup(TADR_FS_G1_BASE, &GPIO_Init_Data, FS_DM_PINNO);
		gpio_setup(TADR_FS_G1_BASE, &GPIO_Init_Data, FS_DP_PINNO);

#if defined(FS_ID_PINNO)
		/*USB ID */
		GPIO_Init_Data.mode      = GPIO_MODE_AF;
		...
		GPIO_Init_Data.speed     = GPIO_SPEED_HIGH;
		GPIO_Init_Data.alternate = GPIO_AF10_OTG_FS;
		gpio_setup(TADR_FS_G1_BASE, &GPIO_Init_Data, FS_ID_PINNO);
#endif

#if defined(FS_VBUS_PINNO)
		/* Configure VBUS Pin(DEVICE) */
		GPIO_Init_Data.mode      = GPIO_MODE_INPUT;
		GPIO_Init_Data.pull      = GPIO_NOPULL;
		GPIO_Init_Data.otype     = GPIO_OTYPE_PP;
		gpio_setup(TADR_FS_G1_BASE, &GPIO_Init_Data, FS_VBUS_PINNO);
#endif

#if defined(FS_POWERSW_PINO)
		/* Configure POWER_SWITCH IO pin(HOST) */
		GPIO_Init_Data.mode      = GPIO_MODE_OUTPUT;
		GPIO_Init_Data.pull      = GPIO_NOPULL;
#ifdef FS_G2_PORT_TYPE
		GPIO_Init_Data.otype     = FS_G2_PORT_TYPE;
#else
		GPIO_Init_Data.otype     = GPIO_OTYPE_PP;
#endif
		GPIO_Init_Data.speed     = GPIO_SPEED_HIGH;
		gpio_setup(TADR_FS_G2_BASE, &GPIO_Init_Data, FS_POWERSW_PINO);
#endif

		/*
		 *  USB FSクロック設定
		 */
		sil_orw_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_USBFS), RCC_USBFSEN);
		sil_orw_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_SYSFG), RCC_SYSGFGEN);
		tmpreg = sil_rew_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_SYSFG));
		 ...
		 *  GPIO/USBポートクロック起動
		 */
		sil_orw_mem((uint32_t *)ENABLE_HS_BASE, ENABLE_HS_VAL2);
		sil_orw_mem((uint32_t *)ENABLE_HS_BASE, ENABLE_HS_VAL3);
		...
		gpio_setup(TADR_GPIOB_BASE, &GPIO_Init_Data, 14);
		gpio_setup(TADR_GPIOB_BASE, &GPIO_Init_Data, 15);
		...
		GPIO_Init_Data.speed     = GPIO_SPEED_HIGH;
		GPIO_Init_Data.otype     = GPIO_OTYPE_PP;
		gpio_setup(TADR_GPIOH_BASE, &GPIO_Init_Data, 12);
...
#else	/* !TOPPERS_STM32F723_DISCOVERY */
		sil_orw_mem((uint32_t *)ENABLE_HS_BASE, ENABLE_HS_VAL4);
		sil_orw_mem((uint32_t *)ENABLE_HS_BASE, ENABLE_HS_VAL5);
		sil_orw_mem((uint32_t *)ENABLE_HS_BASE, ENABLE_HS_VAL1);
		...
		/* DIR */
		gpio_setup(TADR_HS_G1_BASE, &GPIO_Init_Data, HS_DIR_PINNNO);

		/*
		 *  USB HS-PHYクロック設定
		 */
		sil_orw_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_USBHS), RCC_USBHSUPEN);
		tmpreg = sil_rew_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_USBHS));
#endif	/* !TOPPERS_STM32F723_DISCOVERY */

		/*
		 *  USB HSクロック設定
		 */
		sil_orw_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_USBHS), RCC_USBHSEN);
		tmpreg = sil_rew_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_USBHS));
	...
	if(husb->Init.low_power_enable == 1){
		 ...
		 */
#ifdef EXTI_PR_BASE
		sil_wrw_mem((uint32_t *)EXTI_PR_BASE, USB_OTG_FS_WAKEUP_EXTI_LINE);
...
#endif
		sil_orw_mem((uint32_t *)EXTI_IMR_BASE, USB_OTG_FS_WAKEUP_EXTI_LINE);
	}
...
usbo_deinit(USB_OTG_Handle_t *husb)
{
	if(husb->base == TADR_USB_OTGFS_BASE)
		sil_andw_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_USBFS), RCC_USBFSEN);
	else
		sil_andw_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_USBHS), RCC_USBHSEN);
	return E_OK;
}
...
{
#if defined(FS_EVBUS_PINNO)
	if(husb->base == TADR_USB_OTGFS_BASE){
	 	if(state == 0)
			sil_wrw_mem((uint32_t *)(TADR_GPIOG_BASE+TOFF_GPIO_BSRR), (1<<(FS_EVBUS_PINNO+16)));
		else
			sil_wrw_mem((uint32_t *)(TADR_GPIOG_BASE+TOFF_GPIO_BSRR), (1<<(FS_EVBUS_PINNO)));
	}
#endif
}
...
usbo_hcd_irqhandler(USB_OTG_Handle_t *husb)
{
	...
	uint32_t i = 0, int_status = 0, intsts = 0;

	/*
	 *  許可しない要因の割込みは無視する
	 */
	if((intsts = get_int_status(base, msk)) == 0){
		intsts = sil_rew_mem((uint32_t *)(USBOTG_BASE(base)+TOFF_USBO_GINTSTS));
		syslog_1(LOG_EMERG, "usbo_hcd_irqhandler Error Intrrupt[%08x] !", intsts);
		clear_host_int(base, intsts);
		return;
	}

	if((intsts & USB_OTG_GINTSTS_PXFR_INCOMPISOOUT) != 0){
		/*
		 *  未要求割込み要因クリア
		 */
		clear_host_int(base, USB_OTG_GINTSTS_PXFR_INCOMPISOOUT);
	}
	if((intsts & USB_OTG_GINTSTS_IISOIXFR) != 0){
		/*
		 *  未要求割込み要因クリア
		 */
	...
	if((intsts & USB_OTG_GINTSTS_PTXFE) != 0){
		/*
		 *  未要求割込み要因クリア
		 */
		clear_host_int(base, USB_OTG_GINTSTS_PTXFE);
	}
    if((intsts & USB_OTG_GINTSTS_MMIS) != 0){
		/*
		 *  未要求割込み要因クリア
		 */
	 ...
	 *  HOSTディスコネクト割込み
	 */
	if((intsts & USB_OTG_GINTSTS_DISCINT) != 0){
		/*
		 *  HPRTのクリーンアップ
		 */
		sil_andw_mem((uint32_t*)(USBPRT_BASE(base)), (USB_OTG_HPRT_PENA | USB_OTG_HPRT_PCDET |\
	 ...
	 *  HOSTポート割込み
	 */
	if((intsts & USB_OTG_GINTSTS_HPRTINT) != 0){
	 ...
	 *  Host SOF割込み
	 */
    if((intsts & USB_OTG_GINTSTS_SOF) != 0){
	 ...
	 *  HOSTチャネル割込み
	 */
	if((intsts & USB_OTG_GINTSTS_HCINT) != 0){
	 ...
	 *  RXキューレベル割込み
	 */
	if((intsts & USB_OTG_GINTSTS_RXFLVL) != 0){
