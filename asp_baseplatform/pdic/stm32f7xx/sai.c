...
#define FREQUENCY_44K           44100
#define FREQUENCY_22K           22050
#define FREQUENCY_11K           11025
...
#ifdef TOPPERS_STM32F723_DISCOVERY
	TADR_GPIOI_BASE, TADR_GPIOI_BASE, TADR_GPIOG_BASE, TADR_GPIOG_BASE,
	TADR_DMA2_STM4_BASE, DMA_CHANNEL_3, TADR_DMA2_STM6_BASE, DMA_CHANNEL_3,
#else
...
AUDIO_Handle_t *audio_init(ID id)
{
	 ...
	 */
	if(no == 0)
		exti_func[apcb->inintpin] = audio_exti1_int_handler;
	else
		exti_func[apcb->inintpin] = audio_exti2_int_handler;
	return haudio;
}
...
audio_clockconfig(AUDIO_Handle_t *haudio, uint32_t AudioFreq, void *Params)
{
	 ...
	 *  AUDIO周波数からPLL設定値の取得
	 */
	if((AudioFreq == FREQUENCY_11K) || (AudioFreq == FREQUENCY_22K) || (AudioFreq == FREQUENCY_44K)){
	...
	else /* FREQUENCY_8K, FREQUENCY_16K, FREQUENCY_48K, FREQUENCY_96K */{
		/* I2S clock config
		PLLI2S_VCO: VCO_344M
